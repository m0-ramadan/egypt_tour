@php
    ...
    $hasSeasonPrices = $ncDurations->contains(
        fn($d) => $d->seasonPrices
    ...
    $hasFareAccommodations = $fareAccommodations->contains(
        fn($acc) => $acc->seasons
            ->where('is_active', true)
            ->contains(
...
@if ($hasSeasonPrices || $hasFareAccommodations || $ncAddons->isNotEmpty())
    <section class="content-section" id="pricing-packages">
                ...
                @foreach ($ncDurations as $durationIndex => $duration)
                    @php
                        ...
                        $priceLabels = $activeSeasons
                            ...
                            ->map(function ($item) {
                                return trim(
                                    (string) ($item->cabin?->name ?:
                                    ($item->display_label ?:
                                    ucfirst((string) $item->occupancy_type))),
                                );
                            })
                            ->filter()
                            ->unique()
                    ...
                    @if ($activeSeasons->isNotEmpty())
                        <details class="nc-price-duration" {{ $durationIndex === 0 ? 'open' : '' }}>
                            <summary>
                                ...
                                <span
                                    class="nc-price-seasons">{{ trans_choice(':count Season|:count Seasons', $activeSeasons->count(), ['count' => $activeSeasons->count()]) }}</span>
                                <span class="nc-price-from">
                                    @if ($duration->start_from_price)
                                        {{ __('From') }}:
                                        {{ $duration->currency?->symbol ?: ($package->currency?->symbol ?: '$') }}{{ number_format((float) $duration->start_from_price, 0) }}
                                    @else
                                        {{ __('View Prices') }}
                                    @endif
                                </span>
                                            ...
                                            <tr>
                                                ...
                                                @foreach ($priceLabels as $label)
                                                    ...
                                                    <td>
                                                        @if ($priceItem)
                                                            <span
                                                                class="price-value">{{ $season->currency?->symbol ?: ($package->currency?->symbol ?: '$') }}{{ number_format((float) $priceItem->price, 0) }}</span>
                                                        @else
        ...
        @if ($ncAddons->isNotEmpty())
            <div class="mt-4 pt-3 border-top" style="border-color:rgba(43, 43, 43,.08)!important;">
                <h3 class="fw-bold mb-3"
                    style="color: var(--primary-navy, #2b2b2b); font-family: 'Playfair Display', serif;">
                    <i class="la la-tag me-2"
                        style="color: var(--rich-gold, #F36B0A);"></i>{{ __('Optional Cruise Excursions & Add-ons') }}
                </h3>
                <div class="row g-3">
                    @foreach ($ncAddons as $addon)
                        <div class="col-md-6">
                            <div class="included-box h-100">
                                ...
                                @if ($addon->price !== null)
                                    <strong
                                        style="color: var(--rich-gold, #F36B0A); font-size: 1.05rem;">{{ $addon->currency?->symbol ?: ($package->currency?->symbol ?: '$') }}{{ number_format((float) $addon->price, 2) }}</strong>
                                @endif
                            </div>
                        </div>
                    @endforeach

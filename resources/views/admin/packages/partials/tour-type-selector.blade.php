@php
    ...
    $legacyType =
        $selectedPackageType && !array_key_exists($selectedPackageType, $canonicalTourTypes)
            ? $selectedPackageType
            : null;
@endphp
...
<div class="tour-type-selector-shell" data-tour-type-selector>
    <div class="tour-type-selector-heading">
        <div>
            ...
            <p class="tour-type-selector-copy mb-0">
                {{ admin_t('Select your tour type — this determines duration format, itinerary structure, and pricing options.') }}
            </p>
        </div>
    </div>

    <select id="package_type" name="package_type" class="visually-hidden @error('package_type') is-invalid @enderror"
        data-required-step="1" aria-label="Tour Type">
        <option value="">{{ admin_t('Select Tour Type') }}</option>
        @foreach ($canonicalTourTypes as $value => $meta)
            <option value="{{ $value }}" {{ $selectedPackageType === $value ? 'selected' : '' }}>
                {{ $meta['title'] }}</option>
        @endforeach
        @if ($legacyType)
            <option value="{{ $legacyType }}" selected>
                {{ admin_t('Legacy: :type', ['type' => str_replace('_', ' ', $legacyType)]) }}</option>
        @endif
    </select>
    ...
    <div class="tour-type-card-grid" role="radiogroup" aria-label="Tour Type">
        @foreach ($canonicalTourTypes as $value => $meta)
            <button type="button"
                ...
                data-tour-type-card="{{ $value }}" role="radio"
                aria-checked="{{ $selectedPackageType === $value ? 'true' : 'false' }}">
                <span class="tour-type-card-icon"
                    aria-hidden="true">{{ $selectedPackageType === $value ? '✓' : $meta['icon'] }}</span>
...
@once
    <style>
        .tour-type-selector-shell {
            width: 100%
        }

        .tour-type-selector-copy {
            opacity: .7;
            font-size: 13px
        }
        ...
        .tour-type-card-copy strong {
            font-size: 14px
        }

        .tour-type-card-copy small {
            opacity: .6;
            font-size: 12px;
            line-height: 1.45
        }

        .tour-type-card--day-trip.is-selected {
            border: 2px solid #00d084;
            background: rgba(0, 208, 132, .13)
        }

        .tour-type-card--day-trip.is-selected .tour-type-card-icon {
            background: #00bd73;
            color: white
        }

        .tour-type-card--tour-package.is-selected {
            border: 2px solid #3b82f6;
            background: rgba(59, 130, 246, .13)
        }

        .tour-type-card--tour-package.is-selected .tour-type-card-icon {
            background: #2563eb;
            color: white
        }

        .tour-type-card--nile-cruise.is-selected {
            border: 2px solid #a855f7;
            background: rgba(168, 85, 247, .13)
        }

        .tour-type-card--nile-cruise.is-selected .tour-type-card-icon {
            background: #7e22ce;
            color: white
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function syncTourTypeCards() {
                ...
                cards.forEach(card => {
                    ...
                    if (icon) {
                        const metaIcon = card.dataset.tourTypeCard === 'day_tour' ? '🗺️' : (card.dataset
                            .tourTypeCard === 'travel_package' ? '📦' : '🚢');
                        icon.textContent = selected ? '✓' : metaIcon;
                    }
                });

                document.querySelectorAll('[data-tour-type-section]').forEach(section => {
                    const allowed = String(section.dataset.tourTypeSection || '').split(',').map(v => v
                        .trim()).filter(Boolean);
                    ...
                    section.querySelectorAll('input,select,textarea,button').forEach(el => {
                        if (!active) {
                            if (!el.dataset.tourTypeDisabled) {
                                el.dataset.tourTypeDisabled = el.disabled ? 'already' :
                                    'conditional';
                            }
                            el.disabled = true;
                        } else {
                ...
                if (categorySelect) {
                    Array.from(categorySelect.options).forEach(opt => {
                        ...
                        const catType = opt.dataset.categoryType || '';
                        const match = !type || catType === type || (type === 'day_tour' && (catType ===
                            'day_tour' || catType === 'shore_excursion'));
                        opt.hidden = !match;

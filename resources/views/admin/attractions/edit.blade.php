...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0 text-white fw-bold"><i class="fas fa-edit me-2 text-warning"></i>Edit Attraction:
                {{ adminTrans($attraction->name) }}</h4>
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    ...
                    @php
                        ...
                        $rawShortDesc =
                            $attraction->getRawOriginal('short_description') ?? $attraction->short_description;
                        ...
                        if (is_string($rawShortDesc)) {
                            $decoded = json_decode($rawShortDesc, true);
                            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                $rawShortDesc = $decoded;
                            }
                        }
                        ...
                        if (is_string($rawSeoTitle)) {
                            $decoded = json_decode($rawSeoTitle, true);
                            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                $rawSeoTitle = $decoded;
                            }
                        }
                        if (is_string($rawSeoDesc)) {
                            $decoded = json_decode($rawSeoDesc, true);
                            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                $rawSeoDesc = $decoded;
                            }
                        }
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea name="short_description[{{ $code }}]" class="form-control bg-dark text-white border-secondary"
                                                rows="3" dir="{{ $info['dir'] }}" placeholder="Brief summary of the attraction">{{ old('short_description.' . $code, $valShortDesc) }}</textarea>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea name="description[{{ $code }}]" class="form-control rich-editor" data-lang="{{ $code }}"
                                                rows="8" dir="{{ $info['dir'] }}" placeholder="Full detailed description...">{{ old('description.' . $code, $valDesc) }}</textarea>
                                        </div>
                                        ...
                                        <div class="border-top border-secondary pt-3 mt-4">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="seo_title[{{ $code }}]"
                                                    ...
                                                    dir="{{ $info['dir'] }}" placeholder="Meta title for search engines">
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <textarea name="seo_description[{{ $code }}]" class="form-control bg-dark text-white border-secondary"
                                                    rows="2" dir="{{ $info['dir'] }}" placeholder="Meta description for search engine snippets">{{ old('seo_description.' . $code, $valSeoDesc) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" name="is_featured" value="1"
                                    id="is_featured" {{ old('is_featured', $attraction->is_featured) ? 'checked' : '' }}>
                            ...
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" name="is_active" value="1"
                                    id="is_active" {{ old('is_active', $attraction->is_active) ? 'checked' : '' }}>
